...
def plot_graph(data, out_filepath, lb_freq_start=0.01, lb_freq_end=4.0, lb_freq_num=100000, to_display=False, save_to_disk=True):
    ...
    # Calculate as Fedor Semenovich
    xar = np.asarray(x)
    yar = np.asarray(y)
    fsb_ls = LombScargle(xar,yar, normalization='psd')
    fsb_freq, fsb_power = fsb_ls.autopower()
    fsb_false_alarm_probability = fsb_ls.false_alarm_probability(fsb_power.max(), method='baluev')

    # Create figure with 3 subplots
    fig = plt.figure()
    source_ax = fig.add_subplot(311)
    pgram_ax = fig.add_subplot(312)
    fsb_pgram_ax = fig.add_subplot(313)
    ...
    pgram_ax.set_xbound(lower=0.0, upper=20.0)

    # Fedor Semenovich periodogram
    fsb_pgram_ax.plot(2.*np.pi*fsb_freq[:600], fsb_power[:600],'g')
    fsb_pgram_ax.set_xbound(lower=0.0, upper=20.0)
    fsb_pgram_ax.text(0.95, 0.95, "FAP(first_peak) = {:.4f}%".format(fsb_false_alarm_probability),
        verticalalignment='top', horizontalalignment='right',
        transform=fsb_pgram_ax.transAxes,
